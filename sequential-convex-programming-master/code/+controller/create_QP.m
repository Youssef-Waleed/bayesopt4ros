function [n_vars, idx_x, idx_u, idx_slack, objective_quad, objective_lin,...
    A_ineq, b_ineq, A_eq, b_eq, bound_lower, bound_upper] = ...
        create_QP(...
            ...
            iter, i_vehicle,...
            obstacleTable, blockingTable, vhs_ws, new_Q, new_R, new_B)
...
if vh_cfg.approximationIsSL
    objective_lin(idx_pos(vhP.Hp, :)) = new_Q * -vhP.Q * checkpoints(checkpoint_indices(vhP.Hp)).forward_vector;
%       objective_lin(idx_pos(vhP.Hp, :)) = new_Q * -1* checkpoints(checkpoint_indices(vhP.Hp)).forward_vector;
elseif vh_cfg.approximationIsSCR
    objective_lin(idx_pos(vhP.Hp, :)) = new_Q * -vhP.Q * track_polygons(track_polygon_indices(vhP.Hp)).forward_direction;
%     objective_lin(idx_pos(vhP.Hp, :)) = new_Q * -1 * track_polygons(track_polygon_indices(vhP.Hp)).forward_direction;
end

% Minimize control change over time
% create repetetive structure
for i = 1:vhP.Hp - 1
    objective_quad(idx_u(  i, :), idx_u(  i, :)) = new_R * 2 * vhP.R;  
    objective_quad(idx_u(  i, :), idx_u(i+1, :)) = new_R * -vhP.R; 
    objective_quad(idx_u(i+1, :), idx_u(  i, :)) = new_R * -vhP.R;
%     objective_quad(idx_u(  i, :), idx_u(  i, :)) = new_R * 2;  
%     objective_quad(idx_u(  i, :), idx_u(i+1, :)) = new_R; 
%     objective_quad(idx_u(i+1, :), idx_u(  i, :)) = new_R;
end
% overwrite repetetive for first & last entry
objective_quad(idx_u(1,:), idx_u(1,:)) = new_R * vhP.R;
objective_quad(idx_u(vhP.Hp, :), idx_u(vhP.Hp, :)) = new_R * vhP.R;
% objective_quad(idx_u(1,:), idx_u(1,:)) = new_R;
% objective_quad(idx_u(vhP.Hp, :), idx_u(vhP.Hp, :)) = new_R;
...
if vhP.isBlockingEnabled
    ...
    if blockingTable(i_vehicle,1) == 1
        for k = 1:n_affectedTrajSteps
            ...
            objective_quad(idx_pos(k,1),idx_pos(k,1)) = new_B^2 * 64 * ( n1^4 + n1^2 * n2^2 );
            objective_quad(idx_pos(k,2),idx_pos(k,2)) = new_B^2 * 64 * ( n1^2 * n2^2 + n2^4 );
            objective_quad(idx_pos(k,1),idx_pos(k,2)) = new_B^2 * 64 * ( n1^3 * n2 + n1 * n2^3 );
            objective_quad(idx_pos(k,2),idx_pos(k,1)) = new_B^2 * 64 * ( n1^3 * n2 + n1 * n2^3 );
            objective_lin(idx_pos(k,1)) = new_B * 8 * ( (-2)*T1*n1^4 + (-2)*T2*n1^3*n2 + (-2)*T1*n1^2*n2^2 + (-2)*T2*n1*n2^3 + (-2)*n1^3*p1o*n1o + (-2)*n1*n2^2*p1o*n1o + 2*n1^3*T1o*n1o + 2*n1*n2^2*T1o*n1o + (-2)*n1^3*p2o*n2o + (-2)*n1*n2^2*p2o*n2o + 2*n1^3*T2o*n2o + 2*n1*n2^2*T2o*n2o );
            objective_lin(idx_pos(k,2)) = new_B * 8 * ( (-2)*T1*n1^3*n2 + (-2)*T2*n1^2*n2^2 + (-2)*T1*n1*n2^3 + (-2)*T2*n2^4 + (-2)*n1^2*n2*p1o*n1o + (-2)*n2^3*p1o*n1o + 2*n1^2*n2*T1o*n1o + 2*n2^3*T1o*n1o + (-2)*n1^2*n2*p2o*n2o + (-2)*n2^3*p2o*n2o + 2*n1^2*n2*T2o*n2o + 2*n2^3*T2o*n2o );
        end
    end
end
